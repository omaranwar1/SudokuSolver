...
def score_cell_against_templates(
    cell: np.ndarray,
    templates: dict[int, list[np.ndarray]],
    size: int = 50,
) -> dict[str, object]:
    """
    Debug helper: score a cell against all templates and return per-digit scores.

    Returns dict with: best_digit, best_score, second_best, per_digit (dict), candidates (list[np.ndarray])
    """
    # Ensure grayscale
    if cell.ndim == 3:
    ...
    # Otsu binarization + small clean
    _, binary = cv2.threshold(work, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    kernel = np.ones((2, 2), np.uint8)
    binary = cv2.morphologyEx(binary, cv2.MORPH_CLOSE, kernel, iterations=1)
    binary = cv2.morphologyEx(binary, cv2.MORPH_OPEN, kernel, iterations=1)

    # Clear borders to remove grid bleed
    border = max(2, int(min(binary.shape) * 0.08))
    binary[:border, :] = 0
    binary[-border:, :] = 0
    binary[:, :border] = 0
    binary[:, -border:] = 0

    # Connected components to keep largest blob
    ...
    if num_labels <= 1:
        return {
            ...
            "best_score": 0.0,
            "second_best": 0.0,
            "per_digit": {d: 0.0 for d in range(1, 10)},
            "candidates": [binary],
        }
    ...
    digit_norm = cv2.resize(square, (size, size), interpolation=cv2.INTER_AREA)

    candidates: list[np.ndarray] = [digit_norm, cv2.bitwise_not(digit_norm)]
    ...
    for cand in candidates:
        for digit, tmpl_list in templates.items():
            for tmpl in tmpl_list:
                score = cv2.matchTemplate(cand, tmpl, cv2.TM_CCOEFF_NORMED)[0][0]
                per_digit[digit] = max(per_digit[digit], float(score))
                if score > best_score:
...
def _iou(a: np.ndarray, b: np.ndarray) -> float:
    """Binary Intersection-over-Union score."""
    a_bin = a > 127
    b_bin = b > 127
