...
def reinforce_grid_morphological(binary_image, output_size=450):
    ...
    # Step 1: Try to detect existing lines using morphology
    ...
    # Step 3: Create a template grid for missing lines
    # Check each grid line position and add it if missing
    grid_mask = np.zeros_like(binary_image)

    # Check if grid is COMPLETELY empty (no extended lines detected)
    total_extended_pixels = np.sum(horizontal_extend > 0) + np.sum(vertical_extend > 0)

    if total_extended_pixels < 1000:
        # Grid is completely missing - draw full template grid
        ...
        # Grid has some lines - only fill missing ones
        print(f"        Grid partially present - filling missing lines only")

        # Add horizontal lines at expected positions
        # ONLY fill if line is TRULY missing (< 10% coverage)
        for i in range(10):
            y_pos = i * step_y
            # Check if line exists at this position
            y_start = max(0, y_pos - 2)
            y_end = min(h, y_pos + 3)
            line_region = horizontal_extend[y_start:y_end, :]

            # If line is almost completely missing (< 10% coverage), draw it
            if line_region.size > 0:
                coverage = np.sum(line_region > 0) / (line_region.shape[0] * w)
                if coverage < 0.10:
                    # Draw the missing line
                    cv2.line(grid_mask, (0, y_pos), (w-1, y_pos), 255, 2)

        # Add vertical lines at expected positions
        # ONLY fill if line is TRULY missing (< 10% coverage)
        for i in range(10):
            x_pos = i * step_x
            # Check if line exists at this position
    ...
    # Step 6: Combine cleaned digits with reinforced grid
...
def refine_corners_locally(corners, contour, search_radius=15):
    ...
    """
    # Get bounding box of contour to create a mask
    ...
    for corner in corners:
        ...
        # Define search window (relative to mask)
        cx_mask = cx - x_min
        cy_mask = cy - y_min
...
def remove_line_outliers(clusters, image_dimension=None):
    ...
    # Try all possible starting points and find the best group of 10+ lines
    best_group = clusters
    best_score = float('inf')
    min_span_required = image_dimension * 0.65 if image_dimension else 0  # Require 65% span to avoid text/borders

    # Scan through potential starting positions
    for start_idx in range(len(clusters) - 9):  # Need at least 10 lines
        # Try to find a group of evenly-spaced lines starting from this position
        group = [clusters[start_idx]]
        gaps = []

        # Build group of consecutive lines with similar spacing
        for i in range(start_idx + 1, len(clusters)):
            ...
            if len(gaps) == 0:
                ...
                # Check if this gap is similar to previous gaps (within 50% tolerance)
                median_gap = np.median(gaps)
                if 0.5 * median_gap <= gap <= 1.5 * median_gap:
                    group.append(clusters[i])
                    gaps.append(gap)
                elif gap > 1.5 * median_gap:
                    # Large gap - end of evenly-spaced group
                    break

        # Score this group: prefer groups with ~10 lines and consistent spacing
        if len(group) >= 10:
            span = group[-1] - group[0]

            # Skip groups that don't span a significant portion of the image
            # (These are likely noise or partial grids)
            if image_dimension and span < min_span_required:
                continue

            gap_std = np.std(gaps) if len(gaps) > 1 else 0
            # Lower score is better: penalize for deviation, wrong count, and small span
            score = gap_std + abs(len(group) - 10) * 10

            # Bonus for larger span (prefer grids that fill more of the image)
            if image_dimension:
...
def detect_grid_with_hough(processed_image):
    ...
    # - theta: 1 degree resolution (np.pi/180)
...
def detect_blocked_corners(corners_contour, corners_lines, processed_image, debug=False):
    ...
    # If any corner is displaced by more than 6% of image diagonal, suspect occlusion
    if displacement_ratio > 0.06:
        if debug:
